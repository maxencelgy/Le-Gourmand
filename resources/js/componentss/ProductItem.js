...
const ProductItem = ({ product }) => {
    return (
        <Link href={route('product', { slug: product.slug })}>
            <div className="max-w-[280px] group hover:bg-yellow-500 duration-150 rounded-lg bg-[#F5F5F5] flex flex-col justify-between py-6 px-4 h-full shadow-md">
                <div className="flex flex-col items-center flex-grow">
                    ...
                    />
                    <h2 className="font-bold text-black text-center text-xl">{product.name}</h2>
                    <p className="font-light mb-4 text-sm text-center text-black line-clamp-2">
                        {product.description_min}
                    </p>
                </div>
                <div className="flex items-center justify-center group-hover:bg-purple-500 group-hover:text-white fill-current text-purple-500 shadow-md py-2 px-6 bg-yellow-400 flex rounded-3xl items-center">
                    <svg className="w-5 stroke-current group-hover:stroke-white" width="29" height="29" viewBox="0 0 29 29" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                    ...
                    <div className="font-bold ml-4 text-sm leading-none">Ajouter au panier</div>
                </div>
            </div>
        </Link>
    );
